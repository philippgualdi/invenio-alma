...
@click.option(
    "--url", type=click.STRING, default="https://repository.tugraz.at/marc21/{recid}"
)
def update_url_in_alma(user_email, url):
    ...
    :params url (str): new repository url. Musst contain '{recid}'
    """
    identity = get_identity_from_user_by_email(email=user_email)
    current_alma.alma_service.update_url(identity, url)
